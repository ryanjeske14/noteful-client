...
export default class AddNote extends Component {
  ...
  validateContent(fieldValue) {
    ...
    fieldValue = fieldValue.trim();
    if(fieldValue.length === 0) {
      fieldErrors.content = 'Note Name is required';
      hasError = true;
    }
    else {
      if(fieldValue.length < 3) {
        fieldErrors.content = "Note Name must be at lest 3 characters long";
        hasError = true;
      }
      else {
        fieldErrors.content = '';
  ...
  formValid() {
    this.setState({
      formValid: this.state.noteNameValid && this.state.contentValid && this.state.folderValid
    })
  }

  handleSubmit = event => { 
    event.preventDefault();
    const {'note-name-input': noteName, 'note-content-input': content, 'note-folder-select': folder} = event.target;
    const note = {
      id: `${this.makeid(8)}-ffaf-11e8-8eb2-f2801f1b9fd1`,
      name: noteName.value,
      modified: new Date(),
